...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 {
		...
		for si := 0; si < cfg.n; si++ {
			...
			var rf *Raft
			//正常
			//fmt.Println("cfgnis",cfg.n)
			cfg.mu.Lock()
			if cfg.connected[starts] {
				rf = cfg.rafts[starts]
			//	fmt.Println("ask server",starts)
			}
			cfg.mu.Unlock()
			if rf != nil {
				...
				if ok {
					//找到leader
					//fmt.Println("one find leader",starts)
					index = index1
					break
				}
			}else{
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				//index和保存的是不是一个啊嗯啊
				//fmt.Println("index we get is",index)
		//		fmt.Println("will read sth")
				nd, cmd1 := cfg.nCommitted(index)
	
				if nd > 0 && nd >= expectedServers {
					// committed
					if cmd1 == cmd {
					//	fmt.Println("两个cmd什么区别")
			...
			//fmt.Println("get index -1") //but why?
