...
type reqMsg struct {
	...
	args     []byte		//为什么不是reflect.Value类型
	replyCh  chan replyMsg //通过rpc传过去的channel有什么用呢？
	//没关系 一切都快结束了 没关系 马上就可以逃出来了
}
...
type ClientEnd struct {
	...
	ch      chan reqMsg   // copy of Network.endCh  这个copy怎么实现的哈？rpc框架就负责将两端的这个channel保持一致？
	done    chan struct{} // closed when Network is cleaned up
							//相当于是一个数据端口 一个命令端口
							//chanel也是传过去的一个容器 
							//就算不在同一个机器的话 也还是可以用通过rpc使用这些的
}
...
func (e *ClientEnd) Call(svcMeth string, args interface{}, reply interface{}) bool {
	...
	//调用方可以修改传入的两个参数呐
	rep := <-req.replyCh
	//谁会去检查e.ch中的请求 并将回答写入req.reply啊？
	//这个ch是被客户端和服务端共同知道的 怎么做到？
	if rep.ok {
...
type Network struct {
	...
	endCh          chan reqMsg					//相当于请求消息的缓冲区 但为什么在Network这个struct中呢
	done           chan struct{} // closed when Network is cleaned up
...
func MakeNetwork() *Network {
	rn := &Network{} 		//声明一个地址类型竟然是直接在类型前面写&啊？
	rn.reliable = true		//不管是类型本身还是类型地址 访问竟然都是点号啊！
...
//怎么endname关联的信息都记录在rn啊
//related network?
//这相当于服务中心？
//rn是rpc服务提供方？rpc调用需要等待这个服务？
func (rn *Network) processReq(req reqMsg) {
	...
	if enabled && servername != nil && server != nil {
		...
		go func() {
			r := server.dispatch(req) //server是什么？dispatch又是什么 把req拿走又是做什么呢？
			ech <- r
		}()
